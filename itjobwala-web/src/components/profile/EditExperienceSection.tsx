          ...
          <div key={exp.id} className="rounded-2xl border border-gray-100 bg-gray-50 p-4 hover:border-primary/20 transition-colors">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              ...
              <div className="grid grid-cols-2 gap-3">
                <TextField label="Start date" value={formatDateForInput(exp.start_date)} onChange={value => update(exp.id, { start_date: value })} type="date" />
                <TextField label="End date" value={formatDateForInput(exp.end_date)} onChange={value => update(exp.id, { end_date: value })} type="date" disabled={exp.is_current} />
              </div>
            </div>
