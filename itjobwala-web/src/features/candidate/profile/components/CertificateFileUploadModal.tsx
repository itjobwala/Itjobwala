...
interface Props {
  ...
  isUploading: boolean;
  currentFile?: { file_name: string; url: string };
}
...
export default function CertificateFileUploadModal({
  isOpen,
  onClose,
  certificationName,
  onUpload,
    ...
    >
      <div className="bg-white rounded-2xl max-w-md w-full shadow-2xl">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          <div>
            <h2 id="cert-upload-title" className="text-[16px] font-extrabold text-[#0f172a]">Upload Certificate</h2>
            <p className="text-[12px] text-gray-500 mt-1">{certificationName}</p>
          </div>
          <button
            onClick={onClose}
            disabled={isUploading}
            aria-label="Close"
            className="text-gray-400 hover:text-gray-600 disabled:opacity-50 transition-colors shrink-0"
          >
        ...
        <div className="p-6 space-y-4">
          {currentFile && (
            <div className="bg-blue-50 border border-blue-100 rounded-lg p-3">
              <p className="text-[12px] text-blue-900 font-medium">
                Current file: <span className="font-bold">{currentFile.file_name}</span>
              </p>
            </div>
          )}
          ...
          <p className="text-[12px] text-gray-500 text-center">
            Formats: PDF, JPG, JPEG, PNG • Max 5 MB
          </p>
        </div>
