  ...
  return (
    <Card padding="none" className="p-6 sm:p-8" overflow>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-extrabold text-heading" style={{ letterSpacing: '-0.3px' }}>Education</h2>
        {education.length > 0 ? (
          <Button
            variant="ghost"
            size="sm"
      ...
      {education.length === 0 ? (
        ...
        >
          <span className="text-sm font-semibold">Add your education</span>
        </button>
      ) : (
        <div className="flex flex-col gap-4">
          {education.map(ed => (
            <div key={ed.id} className="rounded-2xl border border-token bg-surface-alt p-4 hover:border-primary/20 transition-colors group relative">
