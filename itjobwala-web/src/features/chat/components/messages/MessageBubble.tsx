      ...
      <div
        className={`max-w-[72%] px-4 py-2.5 text-[13.5px] leading-[1.55] break-words ${
          isMine
            ...
            : 'bg-surface text-heading border border-token shadow-sm rounded-[20px] rounded-bl-[4px]'
        }`}
      >
        {message.message}
      </div>
      {isLast && (
        <span className={`text-[10px] text-subtle mx-1 flex items-center gap-1 ${isMine ? 'flex-row-reverse' : ''}`}>
