    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-3">
            {party?.photo ? (
              ...
              <div className="w-11 h-11 rounded-full bg-gradient-to-br from-primary to-blue-400 flex items-center justify-center text-white text-[15px] font-bold shrink-0">
        ...
        {request.referral_job && (
          <div className="text-[12px] text-gray-500 mb-3 flex items-center gap-2 flex-wrap">
            <span className="font-semibold text-gray-700">{request.referral_job.job_title}</span>
            <span>·</span>
        ...
        {request.message && (
          <button
            ...
            className="w-full text-left text-[12px] text-gray-600 bg-gray-50 rounded-xl px-3 py-2.5 mb-3 hover:bg-gray-100 transition-colors border border-gray-100"
          >
        ...
        {!isReceived && request.status === 'accepted' && (
          <div className="space-y-2.5 mb-3">
            ...
            {request.notes && (
              <div className="text-[12px] text-indigo-800 bg-indigo-50 border border-indigo-200 rounded-xl px-4 py-3">
            ...
            {request.apply_link ? (
              <>
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-[14px] font-extrabold bg-emerald-600 text-white hover:bg-emerald-700 active:scale-[0.98] transition-all shadow-sm shadow-emerald-200"
                >
                  Apply Now
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 flex items-center gap-2 text-[12px] text-gray-500">
        ...
        {!isReceived && request.status === 'applied' && (
          <div className="mb-3 space-y-2">
            <div className="bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 flex items-start gap-2.5">
              ...
              <div>
                <p className="text-[12px] font-bold text-teal-800">Application submitted!</p>
                <p className="text-[11px] text-teal-600 mt-0.5">The referrer has been notified that you applied.</p>
              </div>
            </div>
            {request.apply_link && (
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-[11px] text-gray-400 hover:text-gray-600 underline underline-offset-2 break-all"
              >
        ...
        {isReceived && request.status === 'accepted' && request.apply_link && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 mb-3">
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1 text-[12px] font-medium text-blue-700 hover:text-blue-900 underline underline-offset-2 break-all"
            >
        ...
        {isReceived && request.status === 'applied' && (
          <div className="mb-3 space-y-2">
            <div className="bg-teal-50 border border-teal-200 rounded-xl px-4 py-3 flex items-center gap-2">
              ...
              <p className="text-[12px] font-bold text-teal-800">Candidate has applied at your company!</p>
            </div>
            {request.apply_link && (
              <div className="text-[11px] text-gray-500 px-1">
                <span className="font-semibold text-gray-600">Via: </span>
                <a
                  href={request.apply_link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="underline underline-offset-2 hover:text-gray-700 break-all"
                >
                  {request.apply_link}
                </a>
              </div>
        ...
        {isReceived && request.notes && (request.status === 'pending' || request.status === 'accepted') && (
          <div className="text-[11px] text-gray-600 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2 mb-3">
        ...
        <div className="flex gap-2 mb-3">
          {request.resume_url && (
            <a href={request.resume_url} target="_blank" rel="noopener noreferrer"
              className="text-[11px] font-semibold text-primary bg-primary/8 px-3 py-1.5 rounded-lg hover:bg-primary/15 transition-colors flex items-center gap-1">
          ...
          {request.linkedin_url && (
            <a href={request.linkedin_url} target="_blank" rel="noopener noreferrer"
              className="text-[11px] font-semibold text-[#0A66C2] bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition-colors flex items-center gap-1">
          ...
          <div className="flex gap-2 flex-wrap">
            {nextStatuses.map(s => (
              <button
                ...
                className={`text-[12px] font-semibold px-4 py-2 rounded-xl transition-all ${ACTION_STYLES[s] ?? 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
              >
                {ACTION_LABELS[s] ?? s}
              </button>
            ))}
        ...
        {confirmStatus && (
          <div className="bg-gray-50 rounded-2xl px-4 py-3 space-y-3 border border-gray-100">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full text-[12px] border rounded-xl px-3 py-2 focus:outline-none focus:ring-1 transition-all placeholder:text-gray-400 ${
                  applyLinkErr ? 'border-red-400 focus:border-red-400 focus:ring-red-200' : 'border-gray-200 focus:border-primary/50 focus:ring-primary/20'
                }`}
              />
              {applyLinkErr && <p className="text-[10px] text-red-500 mt-0.5">{applyLinkErr}</p>}
              <p className="text-[10px] text-gray-400 mt-1">The candidate will use this link to apply at your company.</p>
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={2}
                className="w-full text-[12px] border border-gray-200 rounded-xl px-3 py-2 resize-none focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all placeholder:text-gray-400"
              />
            </div>
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex-1 text-[12px] font-bold text-white bg-primary px-3 py-2 rounded-xl hover:bg-primary/90 disabled:opacity-50 transition-all"
              >
                {updateMutation.isPending ? 'Sending…' : 'Accept & Send Link'}
              </button>
              <button
                onClick={() => { setConfirm(null); setNotes(''); setApplyLink(''); setApplyLinkErr(''); }}
                className="text-[12px] font-semibold text-gray-500 px-4 py-2 rounded-xl hover:bg-gray-200 transition-colors"
              >
                Cancel
              </button>
            </div>
