import { publicClient, ApiError } from '@/src/lib/api/client';
import { useAuthStore } from '@/src/features/auth/session/auth.store';
import { decodeJwtPayload } from '@/src/lib/auth';
import { getInitials } from '@/src/lib/utils/format';
import type { SessionUser } from '@/src/features/auth/session';

type Role = 'candidate' | 'recruiter';

export interface VerifyOtpRequest {
  email: string;
  role:  Role;
  otp:   string;
}

export interface ResendOtpRequest {
  email: string;
  role:  Role;
}

export class ResendCooldownError extends Error {
  retryAfterSeconds: number;
  constructor(message: string, retryAfterSeconds: number) {
    super(message);
    this.name = 'ResendCooldownError';
    this.retryAfterSeconds = retryAfterSeconds;
  }
}

function buildCandidateSession(token: string, email: string): SessionUser {
  const payload = decodeJwtPayload(token);
  const rawName = payload?.name ? String(payload.name) : email.split('@')[0];
  const name = rawName
    .split(/[._-]/)
    .map((w: string) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase())
    .join(' ');
  return {
    email,
    name,
    initials:            getInitials(name),
    role:                'Candidate',
    userRole:            'candidate',
    avatarColorClass:    'from-primary to-blue-400',
    profilePhoto:        '',
    unreadNotifications: 0,
    unreadMessages:      0,
  };
}

/**
 * Verify OTP with the server; on success, persist the session via the auth
 * store (same actions used by signin) so token storage + cookies are set
 * identically to a normal login.
 */
export async function verifyAndLogin({ email, role, otp }: VerifyOtpRequest): Promise<void> {
  const res = await publicClient.post<{ success: boolean; message: string; token?: string }>(
    '/auth/verify-otp',
    { email, role, otp },
  );
  const token = res.data.token;
  if (!token) throw new Error(res.data.message || 'Verification failed');

  if (role === 'candidate') {
    const user = buildCandidateSession(token, email);
    useAuthStore.getState().loginCandidate(token, user);
  } else {
    useAuthStore.getState().loginRecruiter(token);
  }
}

/**
 * Request a new OTP. Throws ResendCooldownError (with retryAfterSeconds) on
 * 429 so the caller can restart the countdown display from the right value.
 */
export async function resendOtp({ email, role }: ResendOtpRequest): Promise<void> {
  try {
    await publicClient.post('/auth/resend-otp', { email, role });
  } catch (err) {
    if (err instanceof ApiError && err.status === 429) {
      const match = err.message.match(/wait\s+(\d+)\s+second/i);
      const seconds = match ? parseInt(match[1], 10) : 30;
      throw new ResendCooldownError(err.message, seconds);
    }
    throw err;
  }
}
