...
import {
  ...
  getJobCategories,
  getHomeStats,
} from '@/features/jobs/shared';
import { saveJob, unsaveJob } from '@/features/candidate/saved-jobs';
...
export function useRecommendedJobsQuery(exclude?: string, enabled = true) {
  return useQuery({
    queryKey: exclude ? [...jobKeys.recommended(), exclude] : jobKeys.recommended(),
    queryFn:  () => getRecommendedJobs(5, exclude),
    enabled,
  });
}
